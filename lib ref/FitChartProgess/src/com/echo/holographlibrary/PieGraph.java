...
import thaisolution.fitchartprogess.R;
...
public class PieGraph extends View implements HoloGraphAnimate {
	...
	public void onDraw(Canvas canvas) {
		...
		if (textContent != null) {
			String[] sub = textContent.split("\n");
			for (int i = 0;i<sub.length;i++){
				if (mBackgroundImageCenter)
					mBackgroundImageAnchor.set(
							getWidth() / 2 - mBackgroundImage.getWidth() / 2,
							getHeight() / 2 - mBackgroundImage.getHeight() / 2);
				canvas.drawText(sub[i], mBackgroundImageAnchor.x - 50,
						mBackgroundImageAnchor.y + 50*(i), mPaint);
			}
	...
	public void setBackgroundText(String text, int pos_x,
			int pos_y) {
		textContent = text;
		mBackgroundImageAnchor.set(pos_x, pos_y);
		postInvalidate();
